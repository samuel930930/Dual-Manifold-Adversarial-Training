...
cfg.classifier.path = cfg.classifier.path + args.model
...
threat = eval_cfg.get('attack', None)
if threat is not None:
    attacker = get_attack(eval_cfg.attack, net)
else:
    threat = eval_cfg.get('attack_list', None)
    attacker = [get_attack(attack, net) for attack in threat.values()]
...
for i, (images, _, labels) in enumerate(progress_bar):
    ...
    if isinstance(attacker, list):
        ...
        with torch.no_grad():
            pred_clean = net(images).argmax(dim=1)
            pred_adv = net(images_adv).argmax(dim=1)
            dist = per_pixel_l2_dist(images, images_adv)

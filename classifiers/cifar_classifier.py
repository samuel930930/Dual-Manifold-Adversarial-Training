...
if __name__ == '__main__':
    ...
    state_dict = torch.load('../runs/classifiers/cifar/cifar_nat.pt')
    print(model)

    header = 'module.model'
    new_state_dict = {}
    for k, v in state_dict['model'].items():
        if k[:len(header)] == header:
            new_state_dict[k.replace('model.', '')] = v

    model.load_state_dict(new_state_dict)
    torch.save(model.module.state_dict(), '../runs/classifiers/cifar/classifier.pt')
